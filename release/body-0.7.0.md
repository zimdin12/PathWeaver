...
26.1.1 and 26.1.2 share one file; 26.2 is a separate build. One difference worth knowing: the checked tier (`compatibilityTier=AUDITED`) works on **26.1.1 and 26.1.2 only**. Its per-mod exemptions are pinned to the exact bytecode they were derived from, and 26.2 ships different bytes, so on 26.2 that tier refuses everything and switches the mod off. The shipped default performs no such check and is unaffected.
