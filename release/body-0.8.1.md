...
Two honest limits on that. A block changed away from the route can open a shorter way through that a reused route will not take, which vanilla does not notice either. And in 0.8.0 and 0.8.1, switching route sharing or the master switch off and on again inside a route's two-second lifetime can retain a route across changes made while it was off, because observation stops with the feature but stored routes are only discarded when the server stops. Leaving the setting alone avoids it entirely; a fix is in the next release.
...
registered mob type to report which are eligible. That takes about a fifth of a second on a large
pack and it runs on the server thread. It was meant to need operator level: the source comment said
so and the 0.7.0 notes said so. It did not. Any player on any server could run it repeatedly.
**Fixed in 0.8.1**, which changes nothing else. `/pathweaver status` stays open to everyone, since
that is the one the docs tell you to run. If you host a multiplayer server on 0.7.0 or 0.8.0, this
is the reason to update.
...
**The route-sharing saving.** This page presented about 3% as a measured gain. It is not established.
The two arms did not run identical mob populations and the arm order was fixed rather than
counterbalanced, so a small difference with overlapping ranges cannot be pinned on the feature. The
12 to 16% hit rate is real; what serving those hits is worth is unknown.

**"Dropped the moment a block changes."** True while the cache is running, not across an off-and-on
toggle. The route sharing section above has the window.

**A setting screen inside the jar still says "never a stale one".** A page edit cannot reach text
compiled into a release. That wording is in the 0.8.0 and 0.8.1 jars and overstates the guarantee in
exactly the way corrected above. The next release changes it.
