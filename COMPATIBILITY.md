...
This table is version-exact. A verdict applies only to the listed artifact on Minecraft **26.1.2**. **On 26.1.1 and 26.2 none of it applies**: every audit compares the running Minecraft version against `26.1.2` and refuses otherwise, so `AUDITED` denies every movement family there and the mod does nothing at that tier. On 26.1.1 that is stricter than the evidence requires — the pinned vanilla classes are byte-identical — and on 26.2 it is not, because those classes genuinely changed. The shipped `UNSAFE` default consults none of this and works on all three. PathWeaver fingerprints audited artifacts and relevant vanilla classes at startup; a changed version, byte, mixin selector, target descriptor, plugin contribution, missing class, or partial bundle fails closed to synchronous pathing.
