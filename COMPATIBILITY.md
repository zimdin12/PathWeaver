...
The verdict interacts with the `compatibilityTier` setting. `AUDITED`, the shipped default and the most conservative tier that exists, honours `SAFE` rows and `STALE-PATH RISK ONLY` rows — the latter is what a Lithium pack needs, and since Fabric API's own interaction module is a `STALE-PATH RISK ONLY` row, what a *stock* Fabric install needs before PathWeaver does anything. `UNSAFE` ignores this table entirely and is not covered by any evidence here.
...
| Fabric events interaction | `5.2.2+07b380be4c` | Two cancellable `HEAD` injections into the exact `BlockStateBase.useItemOn` and `useWithoutItem` descriptors | Not from the ten pinned worker-side classes that were inventoried | **STALE-PATH RISK ONLY** — the non-reachability argument is a bounded sample, not an exhaustive proof: it inventories direct calls from ten pinned classes and does not traverse the whole worker call graph or build a reverse callsite inventory for those two methods. Honoured at `compatibilityTier=AUDITED`, the shipped default, which is what makes a stock Fabric install work at all. | `dc4a15c9250c6d0e5839e5b696792b06869c65f1ab7e71627986d8f9ed247d60` |
