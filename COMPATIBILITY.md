...
This table is verdict-by-artifact, and from 0.7 the deciding evidence is the **bytes**, not a Minecraft version label. Each audit pins by SHA-256 the artifacts and vanilla classes its own proof depends on, and refuses on any mismatch. **On 26.1.1 these verdicts DO apply**: every pinned vanilla class is byte-identical to 26.1.2, so the proofs hold and `AUDITED` behaves exactly as it does there. **On 26.2 they do not**: `PathNavigation`, `WalkNodeEvaluator`, `BlockBehaviour$BlockStateBase` and `Frog$FrogNodeEvaluator` all changed, so the pins refuse and that tier denies every movement family. The shipped `UNSAFE` default consults none of this and works on all three. PathWeaver fingerprints audited artifacts and relevant vanilla classes at startup; a changed version, byte, mixin selector, target descriptor, plugin contribution, missing class, or partial bundle fails closed to synchronous pathing.
...
The verdict interacts with the `compatibilityTier` setting. **The shipped default is `UNSAFE`, which ignores this table entirely and is not covered by any evidence on this page** — see the README for why. `AUDITED`, the opt-in checked tier and the most conservative one that exists, honours `SAFE` rows and `STALE-PATH RISK ONLY` rows — the latter is what a Lithium pack needs, and since Fabric API's own interaction module is a `STALE-PATH RISK ONLY` row, what a *stock* Fabric install needs before PathWeaver does anything.

**A note on `STRICT` and `ALL` below.** Two tier names in the evidence sections no longer exist. `STRICT` was removed in 0.4.0: it honoured only structural proofs, and the exemption covering Fabric API's own interaction module is a bounded call sample rather than a proof, so it denied every install containing Fabric API — which this mod requires. It could not do anything on any pack that has ever existed. Runs recorded at `STRICT` are kept because they are real measurements of a real tier, and several of them are exactly the evidence that it was inert.

**A note on the name `ALL` below.** The widest tier was called `ALL` through 0.3.x and is called `UNSAFE` from 0.4.0, because it never meant "everything" — it waives the compatibility scan, not the rules about which searches are safe to move. The evidence sections further down record runs that really were executed at a tier named `ALL`, and they say so deliberately. Renaming them would make this document claim we measured something we did not.

The tier is read once, when the startup scan computes this evidence, and is frozen for the rest of the process. Saving a different tier from the settings screen persists it and applies on the next launch; it does not re-open or re-close anything in the running server. Making it live would mean atomically rebuilding every denial and revalidating requests already admitted under the old tier, and doing it partially is worse than not doing it: a session started at `UNSAFE` and saved to `AUDITED` would otherwise keep dispatching work `AUDITED` forbids.

| Mod | Exact version tested | What it modifies | Worker can reach it? | Verdict | Evidence hash (artifact SHA-256) |
|---|---|---|---|---|---|
| ServerCore | `1.5.19+26.1.2` | Three redirects in `PathFinder.findPath(PathNavigationRegion, Mob, Set, float, int, float)` replacing stream/map construction with a local map and per-search evaluator scratch | Yes | **SAFE** — exact exemption implemented | `4f7c9fb3b35b364d8f6500d04f0d5481656f617a83bc69742b47f39adff59666` |
| rabbit-pathfinding-fix | `1.4.0` | `PathNavigation.doStuckDetection(Vec3)` and `resetStuckTimeout()` navigation-maintenance injections | No in the pinned submitted search closure; a live worker-marker probe observed zero entries | **SAFE** — exact non-reachability exemption implemented | `4194ecb5993185922615cbb010fd63ee298e19c52a2978aa17d5c0e77e094d3c` |
...
| Fabric events interaction | `5.2.2+07b380be4c` | Two cancellable `HEAD` injections into the exact `BlockStateBase.useItemOn` and `useWithoutItem` descriptors | Not from the ten pinned worker-side classes that were inventoried | **STALE-PATH RISK ONLY** — the non-reachability argument is a bounded sample, not an exhaustive proof: it inventories direct calls from ten pinned classes and does not traverse the whole worker call graph or build a reverse callsite inventory for those two methods. Honoured at `compatibilityTier=AUDITED`, which is what makes a stock Fabric install work at all when checking is switched on. | `dc4a15c9250c6d0e5839e5b696792b06869c65f1ab7e71627986d8f9ed247d60` |
| Farmer's Delight Refabricated | `26.1-3.6.7+refabricated` | Registers a **dynamic** land path-type provider (`AbstractStoveBlock`) | Yes when its provider is registered | **ALLOWED at `AUDITED`** — its provider receives the world and never loads it, so the answers are precomputed and frozen | `25adee6361b37f1e559373bf6aedc90fa62b2da8ab084e3dee53f037ffcac636` |
| Spiky Spikes | `26.1.0` | Registers **static** land path-type providers (fixed enums) | Answers are precomputed, so its code never runs on a worker | **ALLOWED at `AUDITED`** — certified generically, no audit or hash pin needed, but the certification rests on a provider being a pure function of block state | `323d974770988a17c6d054dc879b528c5e5c25ac48241748316bb6aa679eee04` |
...
a structural proof, so it is bounded evidence rather than proof. Lithium
...
provider's. Both forms rest on bounded evidence rather than proof, for different reasons: the
