# Phase 1 diagnostics, v4: one decision, one report

Draft for independent review before anything is implemented. Replaces
`phase1-diagnostics-v3.md` at `18eede7db857c8aabb2aa8a69cca9c4296178a36` (REVISE with scope
accepted, review `phase1-18eede7-verdict.md`), kept unedited, as are v2 (`6fd91cb`) and v1
(`be258c31`). Source locations refer to `080fc42eec1809507305c67e891eb8c45bc85a95`; `src` is unchanged
since.

## Changes from v3

v3's scope stands: one empirical D decision and one client report, with scheduling deferred. v4
repairs the instrument laws the review found incomplete:

- synchronous-search subtraction is defined as outermost-only per bracket, so nested searches are
  not subtracted twice;
- W3 and W4 exposure uses a specified server request sampler that records class and method names,
  not jar code source;
- the client report gets producer ownership, bounded transport, window membership and finalization;
- boundaries, tick denominators, state precedence, relative-difference and zero rules, and totals
  folding are stated;
- D's exclusions and the meaning of STOP are narrowed as the review asked.
...
every state. A mark takes effect on the server thread at `END_SERVER_TICK`, immediately **before** the
`onEndTick` bracket is entered. At that instant it:
...
2. records the tick number, a `System.nanoTime()` stamp, and a snapshot of PathWeaver's outcome
   counters read directly from `PathWeaverRuntime`, not parsed from status text;
3. in RECORDING only, opens or closes the accumulation window.

The capture interval is **half-open**, from boundary A (inclusive) to boundary B (exclusive). It holds
`onEndTick` of tick A through everything in tick B before the boundary: exactly **B − A** tick
periods. Every per-tick quantity uses B − A as its denominator. B − A < 20 voids the window.

**Tick timing.** From mark begin to mark end, in every state, the diagnostic jar records the duration
of each server tick whose `END_SERVER_TICK` falls after boundary A and at or before boundary B. That is
B − A durations, from the server's own tick-time measurement. Mean and p95 come from those
durations. The accumulator is identical across states, so it adds no difference between them.
