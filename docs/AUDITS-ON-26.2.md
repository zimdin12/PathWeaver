...
verify on 26.2 and the scan reports `deniedFamilies=0`. The field-write proof is `tools/audit_field_writes.py`,
which reads every field write in the audited classes and requires each one to sit in `<init>`, `<clinit>`
or a named Lithium initialiser.
