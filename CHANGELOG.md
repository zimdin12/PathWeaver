...
`docs/evidence/client-lag-2026-09-14/` on the master branch.
...
- **Failure attribution, which is the half that pays on every install.** The breaker may never fire —
  the reference pack ran 753 dispatches with zero search failures. The attribution fires on the
...
- **The startup log says WHY `UNSAFE` is the shipped default.** It already warned that the denial had
  ...
  types** on a 221-jar server pack — so shipping it as the default would ship a mod that does nothing. It is
